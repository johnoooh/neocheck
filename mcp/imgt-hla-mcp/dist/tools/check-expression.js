 ...
 */
import { z } from 'zod';
import { getExpressionSuffix, EXPRESSION_DESCRIPTIONS, } from '../types/imgt.js';
export const checkExpressionSchema = z.object({
    alleles: z.array(z.string()).min(1).describe('List of allele names to check'),
});
export async function checkExpression(input) {
    const { alleles } = input;
    const results = alleles.map((allele) => {
...
export const checkExpressionTool = {
    ...
    description: 'Check if HLA alleles are null (N), low-expressing (L), or have other expression variants (S, C, A, Q)',
    inputSchema: checkExpressionSchema,
    handler: checkExpression,
};
//# sourceMappingURL=check-expression.js.map