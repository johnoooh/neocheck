...
export async function searchCells(input: SearchCellsInput): Promise<SearchCellsResult> {
  ...
  // Try cache first
  const cached = cache.get<SearchCellsResult>(cacheKey);
  if (cached) {
    return cached;
  }
  ...
  const result: SearchCellsResult = {
