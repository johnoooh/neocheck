...
export async function searchReferences(input: SearchReferencesInput): Promise<SearchReferencesResult> {
  const cacheKey = CacheKeys.referenceSearch(JSON.stringify(input));
  const cached = cache.get<SearchReferencesResult>(cacheKey);
  ...
  const references: ReferenceSummary[] = records.map((r) => ({
  ...
  const result: SearchReferencesResult = { references, count: references.length };
...
export const searchReferencesTool = {
  ...
  description:
    'Search publications and references in CEDAR by title, author, PubMed ID, or neoantigen focus. Returns reference summaries with publication details, epitope counts, and PubMed links.',
