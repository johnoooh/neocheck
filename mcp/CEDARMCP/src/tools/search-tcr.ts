...
export async function searchTcr(input: SearchTcrInput): Promise<SearchTcrResult> {
  const cacheKey = CacheKeys.epitopeSearch(`tcr:${JSON.stringify(input)}`);
  const cached = cache.get<SearchTcrResult>(cacheKey);
  ...
  const receptors: TcrSummary[] = records.map((r) => ({
  ...
  const result: SearchTcrResult = { receptors, count: receptors.length };
