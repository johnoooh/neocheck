...
import { z } from 'zod';
import { ambiguityDetector, type AmbiguityResult } from '../analysis/ambiguity-detector.js';
...
export async function findAmbiguousPairs(
  input: FindAmbiguousPairsInput
): Promise<FindAmbiguousPairsResult> {
  const { allele1, allele2, sequence_type, region_start, region_end } = input;

  const result = await ambiguityDetector.findAmbiguousPairs(
    allele1,
    allele2,
    sequence_type,
    region_start,
...
export const findAmbiguousPairsTool = {
  ...
  description:
    'Find HLA allele pairs that are indistinguishable based on sequence identity. ' +
