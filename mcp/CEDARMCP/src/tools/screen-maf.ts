...
import type { EpitopeSearchRecord, TcellExportRecord } from '../types/cedar.js';
...
export const screenMafSchema = z.object({
  maf_file_path: z
    ...
    .optional()
    .describe(
      'Absolute path to a MAF file on disk. Either this or mutations must be provided.'
    ),
  mutations: z
    .array(mafMutationSchema)
    .optional()
    .describe(
      'Pre-parsed array of mutations as an alternative to maf_file_path. Each entry needs hugo_symbol, hgvsp_short, variant_classification.'
    ),
  ...
  include_tcell_details: z
    ...
    .default(false)
    .describe(
      'If true, enrich each epitope hit with detailed T-cell export data including response frequency, subject counts, assay methods, MHC restriction details, and TCR names. Requires additional API calls per hit.'
    ),
});
 ...
 */
async function parseMafFile(filePath: string): Promise<ParsedMafRow[]> {
  ...
  const colIndex = (name: string): number => {
    ...
    if (idx === -1) throw new Error(`Required MAF column not found: ${name}`);
    return idx;
  };
  ...
  const rows: ParsedMafRow[] = [];
  for (let i = 1; i < lines.length; i++) {
    const cols = lines[i].split('\t');
    if (cols.length <= hugoIdx) continue;
    ...
    rows.push({
      ...
      t_var_freq:
        tVarFreqIdx >= 0 && cols[tVarFreqIdx]
 ...
 */
function extractShortMutation(hgvsp: string): string | null {
 ...
 */
function isMissense(varClass: string): boolean {
 ...
 */
function toEpitopeSummary(r: EpitopeSearchRecord): EpitopeSummary {
  return {
 ...
 */
async function queryMutationsBatched(
  mutations: Array<{ shortMutation: string; index: number }>,
  maxPerMutation: number,
  concurrency: number = 3
): Promise<Map<number, EpitopeSummary[]>> {
  const results = new Map<number, EpitopeSummary[]>();

  // Deduplicate mutations to avoid redundant API calls
  const uniqueMutations = new Map<string, number[]>();
  ...
  for (let i = 0; i < uniqueEntries.length; i += concurrency) {
    ...
    const promises = batch.map(async ([shortMutation, indices]) => {
      ...
      const cached = cache.get<EpitopeSummary[]>(cacheKey);

      let summaries: EpitopeSummary[];
      if (cached) {
        summaries = cached;
      } else {
 ...
 */
function toTcellDetail(r: TcellExportRecord): TcellDetailRecord {
  return {
 ...
 */
async function enrichWithTcellDetails(
  epitopeHits: Map<number, EpitopeSummary[]>,
  concurrency: number = 3
): Promise<void> {
  // Collect unique linear_sequences across all hits that have T-cell assays
  const sequenceToStructureIds = new Map<string, Set<number>>();
  ...
  for (let i = 0; i < sequenceList.length; i += concurrency) {
    ...
    const promises = batch.map(async (seq) => {
      ...
      const cached = cache.get<TcellDetailRecord[]>(cacheKey);
      if (cached) {
...
export async function screenMafNeoantigens(
  input: ScreenMafInput
): Promise<MafScreeningResult> {
  // 1. Get mutations from file or direct input
  let parsedRows: ParsedMafRow[];
  ...
  const cedarResults = await queryMutationsBatched(
    missenseMutations.map((m) => ({
  ...
  // 4. Build results
  const mutationResults: MutationScreenResult[] = missenseMutations.map((m) => {
