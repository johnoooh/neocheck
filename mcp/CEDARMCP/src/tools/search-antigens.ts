...
export async function searchAntigens(input: SearchAntigensInput): Promise<SearchAntigensResult> {
  const cacheKey = CacheKeys.antigenSearch(JSON.stringify(input));
  const cached = cache.get<SearchAntigensResult>(cacheKey);
  ...
  const antigens: AntigenSummary[] = records.map((r) => ({
  ...
  const result: SearchAntigensResult = { antigens, count: antigens.length };
...
export const searchAntigensTool = {
  ...
  description:
    'Search for source antigens in CEDAR by name (e.g., KRAS, TP53, BRAF), organism, or neoantigen status. Returns antigen summaries with associated epitope counts and disease contexts.',
