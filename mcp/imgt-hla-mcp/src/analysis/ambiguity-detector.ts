...
import { extractLocus, type SequenceType, type AlleleDetailResponse } from '../types/imgt.js';
...
export class AmbiguityDetector {
   ...
   */
  async findAmbiguousPairs(
    allele1: string,
    allele2: string,
    sequenceType: SequenceType = 'coding',
    regionStart?: number,
    regionEnd?: number
    ...
    const locus1Candidates = this.findSwapCandidates(
      allele1,
      seq1,
      differences,
      locus1Alignment,
    ...
    const locus2Candidates = this.findSwapCandidates(
      allele2,
      seq2,
      differences,
      locus2Alignment ?? locus1Alignment,
      'allele2'
    );
   ...
   */
  private async getAlleleSequence(allele: string, sequenceType: SequenceType): Promise<string> {
   ...
   */
  private async getLocusAlignmentData(
    locus: string,
    sequenceType: SequenceType
  ): Promise<Map<string, string>> {
    const alignmentType =
      sequenceType === 'protein' ? 'protein' : sequenceType === 'genomic' ? 'genomic' : 'nucleotide';

    const cacheKey = `alignment-map:${locus}:${alignmentType}`;
    let alignmentMap = cache.get<Map<string, string>>(cacheKey);
  ...
  private findSwapCandidates(
    ...
    alignmentData: Map<string, string>,
    whichAllele: 'allele1' | 'allele2'
  ): Array<{ allele: string; sequence: string }> {
    const candidates: Array<{ allele: string; sequence: string }> = [];

    for (const [alleleName, sequence] of alignmentData) {
      // Skip the original allele
      if (alleleName === originalAllele) continue;
  ...
  private producesSamePattern(
    ...
    for (const diff of differences) {
      ...
      if (origBases.size !== candBases.size) return false;
      for (const base of origBases) {
        if (!candBases.has(base)) return false;
      }
    }
  ...
  private generateExplanation(
    ...
    origSeq1: string,
    origSeq2: string,
    cand1: { allele: string; sequence: string },
    cand2: { allele: string; sequence: string }
  ): string {
    const parts: string[] = [];
