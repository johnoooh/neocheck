...
export async function searchMhcLigands(input: SearchMhcInput): Promise<SearchMhcResult> {
  const cacheKey = CacheKeys.mhcSearch(JSON.stringify(input));
  const cached = cache.get<SearchMhcResult>(cacheKey);
  ...
  const ligands: MhcLigandSummary[] = records.map((r) => ({
  ...
  const result: SearchMhcResult = { ligands, count: ligands.length };
...
export const searchMhcTool = {
  ...
  description:
    'Search MHC ligand and elution assay data in CEDAR. Filter by peptide sequence, mutation, MHC allele, or qualitative result. Returns binding/elution records with MHC allele, evidence type, assay method, and publication info.',
