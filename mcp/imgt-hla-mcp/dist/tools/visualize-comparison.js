...
export async function visualizeComparison(input) {
    ...
    const { differences: proteinDiffs, skippedUnknown: proteinSkipped } = findDifferencesAligned(alleleDataList.map((a) => ({ name: a.name, seq: a.proteinSeq })));
    const { differences: codingDiffs, skippedUnknown: codingSkipped } = findDifferencesAligned(alleleDataList.map((a) => ({ name: a.name, seq: a.codingSeq })));
 ...
 */
function findDifferencesAligned(sequences) {
...
function generateHTML(title, alleles, proteinDiffs, codingDiffs, locus, warnings) {
...
function generateAlignmentHTML(alleles, seqKey, diffs, blockSize) {
...
export const visualizeComparisonTool = {
    ...
    description: 'Generate an interactive HTML visualization of HLA allele comparisons. ' +
...
//# sourceMappingURL=visualize-comparison.js.map