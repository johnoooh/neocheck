...
import type { SequenceType, AlignedSequence, AlignmentType } from '../types/imgt.js';
...
export async function compareAlleles(input: CompareAllelesInput): Promise<CompareAllelesResult> {
  ...
  const samePGroup =
    uniquePGroups.size === 1 && pGroupInfos.every((p) => p.pGroup !== null);

  let pGroupExplanation: string;
  if (samePGroup) {
  ...
  // Generate summary
  const summary =
    `Compared ${alleles.length} alleles from locus ${locus}:\n` +
...
async function fetchAlignedSequences(
  ...
  const matchedNames = new Map<string, string>(); // requested name -> matched name
 ...
 */
function compareAlignedSequences(
...
export const compareAllelesTool = {
  ...
  description:
    'Compare two or more HLA alleles and identify sequence differences. ' +
